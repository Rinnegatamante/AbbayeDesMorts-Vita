...
/* Gestión del teclado durante el juego */
   ...
   while (SDL_PollEvent(&evento)) {
		...
		if (evento.type == SDL_JOYBUTTONDOWN){
			...
			}else if(evento.jbutton.button==7) { // Left button
				if (jean->pulsa[2] == 0) {
					jean->pulsa[2] = 1;
					jean->pulsa[3] = 0;
				}
			}else if(evento.jbutton.button==9) { // Right button
				if (jean->pulsa[3] == 0) {
					jean->pulsa[3] = 1;
					jean->pulsa[2] = 0;
				}
   	...
   	/*if (evento.type == SDL_KEYDOWN) {
		...
		if (evento.type == SDL_JOYBUTTONDOWN){
			if(evento.jbutton.button==2) { // Cross button
				...
				*tecla = 9;
			}else if(evento.jbutton.button==10) { // Select button
				exit(0);
			}
		}
		#endif
	   	...
	   	/*if (evento.key.keysym.sym == SDLK_ESCAPE)
			...
			if (evento.key.keysym.sym == KEY_GRAPHICS)
				*tecla = 9;
#ifdef _3DS
			if (evento.key.keysym.sym == KEY_START || evento.key.keysym.sym == KEY_ACTION)
#else
...
void tecladop (int *teclap) {
	...
	while (SDL_PollEvent(&evento)) {
		...
		#ifdef _PSP2
		if (evento.type == SDL_JOYBUTTONDOWN){
			if(evento.jbutton.button==11) { // Start button
				*teclap = 1;
			}
		}
		#endif
